...
app.get("/", (_req: Request, res: Response) => {
  res.status(200).type("text/plain").send("Apps Builder MCP server");
});
...
app.listen(appConfig.server.port, () => {
  console.log(
    `Apps Builder MCP server listening on http://localhost:${appConfig.server.port}${appConfig.server.mcpPath}`
  );
});
